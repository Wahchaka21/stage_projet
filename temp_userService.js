...
async function archiveUserData(user, deletionContext) {
  try {
    // 1) Hash “preuve” de l’état du user avant suppression
    const dataHash = crypto
      .createHash('sha256')
      .update(JSON.stringify(user.toObject()))
      .digest('hex')

    // 2) Politique de rétention (ex: 30 jours)
    const days = Number(process.env.DELETE_GRACE_DAYS || DELETION_CONFIG.GRACE_PERIOD_DAYS || 30)
    const keptUntil = new Date(Date.now() + days * 24 * 60 * 60 * 1000)

    // 3) Construire l’archive (avec email masqué)
    const archiveDoc = new UserDeletionArchive({
